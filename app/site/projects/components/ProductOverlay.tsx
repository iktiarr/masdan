...
export default function ProductOverlay({
  ...
  const uniqueData = useMemo(() => {
    if (!Array.isArray(products) || products.length === 0) {
      return { labels: ["Semua"], badges: ["Semua"], jenis: ["Semua"] };
    }
    ...
    for (const p of products) {
      const f = p?.fields;
      if (f?.label) labels.add(String(f.label));
      if (f?.badge) badges.add(String(f.badge));
      if (f?.jenis) jenis.add(String(f.jenis));
    }
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      data = data.filter((item) =>
        String(item?.fields?.name || "").toLowerCase().includes(q)
      );
    }

    if (selectedLabel !== "Semua") {
      data = data.filter((item) => item?.fields?.label === selectedLabel);
    }
    if (selectedBadge !== "Semua") {
      data = data.filter((item) => item?.fields?.badge === selectedBadge);
    }
    if (selectedJenis !== "Semua") {
      data = data.filter((item) => item?.fields?.jenis === selectedJenis);
    }

    data.sort((a, b) => {
      const priceA = Number(a?.fields?.harga ?? 0);
      const priceB = Number(b?.fields?.harga ?? 0);
      const nameA = String(a?.fields?.name ?? "").toLowerCase();
      const nameB = String(b?.fields?.name ?? "").toLowerCase();
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          <div className="shrink-0 bg-white dark:bg-[#0a0a0a] border-b border-gray-200 dark:border-white/10 shadow-sm sticky top-0 z-[50]">
            <div className="flex items-center gap-3 px-4 py-3 md:px-8 border-b border-gray-100 dark:border-white/5">
              ...
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-500" size={18} aria-hidden />
                <input
                  type="text"
                  inputMode="search"
            ...
            <button
              ...
              aria-controls="product-overlay-filters"
            >
              <Filter size={14} />
                ...
                <section role="list" aria-label="Daftar produk">
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-5">
                    {filteredProducts.map((item: any, idx: number) => (
                      <div role="listitem" key={item?.sys?.id ?? idx}>
