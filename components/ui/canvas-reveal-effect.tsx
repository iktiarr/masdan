...
const ShaderMaterial = ({
  ...
  const preparedUniforms = useMemo(() => {
    ...
    for (const key in uniforms) {
      ...
      if (u.type === "uniform1f")
        obj[key] = { value: u.value, type: "1f" };

      else if (u.type === "uniform1fv")
        obj[key] = { value: u.value, type: "1fv" };

      else if (u.type === "uniform3fv")
        obj[key] = { value: (u.value as number[][]).map((v) => new THREE.Vector3(...v)), type: "3fv" };

      else if (u.type === "uniform2f")
        obj[key] = { value: new THREE.Vector2(...(u.value as number[])), type: "2f" };
    }
